...
const verifyClientId = async (client) => {
    const resp = await (0, fetch_js_1.getClientFetch)(client)(constants_js_1.ROUTE_VERIFY_THIRDWEB_CLIENT_ID, {
        body: (0, json_js_1.stringify)({ clientId: client.clientId, parentDomain: "" }),
        headers: {
            ...getSessionHeaders(),
        },
        method: "POST",
...
async function storeUserShares({ client, walletAddress, maybeEncryptedRecoveryShares, authShare, storage, }) {
    ...
    if (!resp.ok) {
        const error = await resp.json();
        throw new Error(`Something went wrong storing user wallet shares: ${(0, json_js_1.stringify)(error.message, null, 2)}`);
    }
}
async function getUserShares(args) {
    ...
    if (!resp.ok) {
        const error = await resp.json();
        throw new Error(`Something went wrong getting user's wallet: ${(0, json_js_1.stringify)(error.message, null, 2)} `);
    }
