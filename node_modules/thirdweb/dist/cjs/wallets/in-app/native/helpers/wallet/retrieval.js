...
exports.getWalletPrivateKeyFromShares = getWalletPrivateKeyFromShares;
...
async function getWalletPrivateKeyFromShares(shares) {
    const { secrets } = await Promise.resolve().then(() => require("./sss.js"));
 ...
 */
async function getShares({ client, authShare, deviceShare, recoveryShare, storage, }) {
    const queryParams = {};
    ...
    const userShares = await (0, fetchers_js_1.getUserShares)({ client, getShareUrl, storage });
...
async function setUpShareForNewDevice({ recoveryCode, client, storage, }) {
