...
async function setUpNewUserWallet(args) {
    ...
    try {
        ...
        await (0, logout_js_1.logoutUser)({ client, storage });
        throw new Error(`Error creating new ews account. Please try signing in again. Original Error: ${e}`);
    }
}
// Wallet Creation
async function generateWallet({ client, recoveryCode, storage, }) {
 ...
 */
async function storeShares({ client, walletAddress, authShare, deviceShare, recoveryShares, storage, }) {
