...
const compiler_metadata_js_1 = require("../actions/compiler-metadata.js");
const constructor_params_js_1 = require("./constructor-params.js");
...
async function verifyContract(options) {
    const implementation = await (0, resolveImplementation_js_1.resolveImplementation)(options.contract);
    const ipfsUri = (0, extractIPFS_js_1.extractIPFSUri)(implementation.bytecode);
    if (!ipfsUri) {
        throw new Error("Contract bytecode does not contain IPFS URI, cannot verify");
    }
    const res = await (0, download_js_1.download)({
        client: options.contract.client,
        uri: ipfsUri,
    });
    const compilerMetadata = (0, compiler_metadata_js_1.formatCompilerMetadata)(await res.json());
    const sources = await (0, source_files_js_1.fetchSourceFilesFromMetadata)({
        client: options.contract.client,
    ...
    if (options.type === "blockscoutV2") {
        ...
        const result = await fetch(`${options.explorerApiUrl}/v2/smart-contracts/${options.contract.address.toLowerCase()}/verification/via/standard-input`, {
            body: formData,
            method: "POST",
        });
    ...
    const encodedArgs = options.encodedConstructorArgs
        ...
        : await (0, constructor_params_js_1.fetchConstructorParams)({
 ...
 * @contract
 */
async function checkVerificationStatus(options) {
