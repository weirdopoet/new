...
function resolveContractAbi(contract, contractApiBaseUrl = "https://contract.thirdweb.com/abi") {
    return (0, withCache_js_1.withCache)(async () => {
        ...
        if (contract.chain.id === 31337 ||
            contract.chain.id === 1337 ||
            contract.chain.id === chains_1.sepolia.id // FIXME remove this once contract API handles 7702 delegation
        ) {
...
contract) {
    const [{ resolveImplementation }, { extractIPFSUri }] = await Promise.all([
        Promise.resolve().then(() => require("../../utils/bytecode/resolveImplementation.js")),
        Promise.resolve().then(() => require("../../utils/bytecode/extractIPFS.js")),
    ]);
    ...
    try {
        const res = await (0, download_js_1.download)({ client: contract.client, uri: ipfsUri });
...
async function resolveModularModuleAddresses(contract) {
    try {
        const { getInstalledModules } = await Promise.resolve().then(() => require("../../extensions/modules/__generated__/IModularCore/read/getInstalledModules.js"));
...
async function getAbisForPlugins(options) {
    return Promise.all(options.plugins.map((pluginAddress) => {
        const newContract = (0, contract_js_1.getContract)({
            ...options.contract,
