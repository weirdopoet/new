"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.eth_getLogs = eth_getLogs;
const viem_1 = require("viem");
const address_js_1 = require("../../utils/address.js");
const hex_js_1 = require("../../utils/encoding/hex.js");
/**
 * Retrieves logs from the Ethereum blockchain based on the specified parameters.
 * @param request - The EIP1193 request function.
 * @param params - The parameters for retrieving logs.
 * @returns A promise that resolves to the retrieved logs.
 * @rpc
 * @example
 * ```ts
 * import { getRpcClient, eth_getLogs } from "thirdweb/rpc";
 * const rpcRequest = getRpcClient({ client, chain });
 * const logs = await eth_getLogs(rpcRequest, {
 *  address: "0x...",
 *  fromBlock: 123456n,
 *  toBlock: 123456n,
 * });
 * ```
 */
async function eth_getLogs(request, params = {}) {
    const topics = params.topics ?? [];
    let logs;
    // in the case we have a blockHash
    if (params.blockHash) {
        const param = {
            blockHash: params.blockHash,
            topics,
        };
        if (params.address) {
            param.address = (0, address_js_1.getAddress)(params.address);
        }
        logs = await request({
            method: "eth_getLogs",
            params: [param],
        });
    }
    // otherwise
    else {
        const param = { topics };
        if (params.address) {
            param.address = params.address;
        }
        if (params.fromBlock) {
            param.fromBlock =
                typeof params.fromBlock === "bigint"
                    ? (0, hex_js_1.numberToHex)(params.fromBlock)
                    : params.fromBlock;
        }
        if (params.toBlock) {
            param.toBlock =
                typeof params.toBlock === "bigint"
                    ? (0, hex_js_1.numberToHex)(params.toBlock)
                    : params.toBlock;
        }
        logs = await request({
            method: "eth_getLogs",
            params: [param],
        });
    }
    return logs.map((log) => (0, viem_1.formatLog)(log));
}
//# sourceMappingURL=eth_getLogs.js.map