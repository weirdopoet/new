...
async function toSerializableTransaction(options) {
    ...
    if (isZkSync) {
        ...
        const { gas, maxFeePerGas, maxPriorityFeePerGas } = await getZkGasFees({
            from: typeof options.from === "string" // Is this just an address?
                ? (0, address_js_1.getAddress)(options.from)
                : options.from !== undefined // Is this an account?
                    ? (0, address_js_1.getAddress)(options.from.address)
                    : undefined,
    ...
    let [data, nonce, gas, feeData, to, accessList, value, authorizationList, type,] = await Promise.all([
        ...
        (async () => {
            ...
            return from // otherwise get the next nonce (import the method to do so)
                ? await Promise.resolve().then(() => require("../../rpc/actions/eth_getTransactionCount.js")).then(({ eth_getTransactionCount }) => eth_getTransactionCount(rpcRequest, {
                    address: typeof from === "string"
                        ? (0, address_js_1.getAddress)(from)
                        : (0, address_js_1.getAddress)(from.address),
                    blockTag: "pending",
                }))
                : undefined;
        })(),
        // takes the same options as the sendTransaction function thankfully!
        (0, estimate_gas_js_1.estimateGas)({
            ...options,
            from: options.from,
        }),
        ...
        (0, resolve_promised_value_js_1.resolvePromisedValue)(options.transaction.type),
    ]);
