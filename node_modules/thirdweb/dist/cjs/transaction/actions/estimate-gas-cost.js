...
async function estimateGasCost(options) {
    ...
    const gasLimit = (await (0, resolve_promised_value_js_1.resolvePromisedValue)(transaction.gas)) ||
        (await (0, estimate_gas_js_1.estimateGas)({ from, transaction }));
    const fees = await (0, fee_data_js_1.getDefaultGasOverrides)(transaction.client, transaction.chain);
    const gasPrice = fees.maxFeePerGas || fees.gasPrice;
    if (gasPrice === undefined) {
        throw new Error(`Unable to determine gas price for chain ${transaction.chain.id}`);
    }
    let l1Fee;
    if (await (0, constants_js_1.isOpStackChain)(transaction.chain)) {
