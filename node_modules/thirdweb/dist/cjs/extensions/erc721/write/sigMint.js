...
async function generateMintSignature(options) {
    ...
    const [price, uri, uid] = await Promise.all([
        ...
        (() => {
            if (mintRequest.uid) {
                return (0, viem_1.isHex)(mintRequest.uid)
                    ? mintRequest.uid
                    : (0, viem_1.stringToHex)(mintRequest.uid, { size: 32 });
            }
    ...
    if (mintRequest.primarySaleRecipient?.length === 0 ||
        ...
        const { primarySaleRecipient } = await Promise.resolve().then(() => require("../../common/__generated__/IPrimarySale/read/primarySaleRecipient.js"));
    ...
    if (mintRequest.royaltyRecipient?.length === 0 ||
        ...
        const { getDefaultRoyaltyInfo } = await Promise.resolve().then(() => require("../../common/__generated__/IRoyalty/read/getDefaultRoyaltyInfo.js"));
