...
import { formatAbiItem, parseAbiItem, } from "abitype";
...
export async function generate(input) {
    ...
    await writeFile(join(chainDirPath, `${contractAddress.toLowerCase()}.ts`), generated);
}
export function isValidChainIdAndContractAddress(chainIdPlusContract) {
...
async function generateFromAbi(abi) {
    ...
    abi = abi.map((x) => (typeof x === "string" ? parseAbiItem(x) : x));
