...
import { AWS_REGION, COGNITO_IDENTITY_POOL_ID, GENERATE_RECOVERY_PASSWORD_LAMBDA_FUNCTION_V1, GENERATE_RECOVERY_PASSWORD_LAMBDA_FUNCTION_V2, ROUTE_AUTH_COGNITO_ID_TOKEN_V1, ROUTE_AUTH_COGNITO_ID_TOKEN_V2, ROUTE_COGNITO_IDENTITY_POOL_URL, } from "../constants.js";
export async function getCognitoRecoveryPasswordV1(args) {
    ...
    if (!idTokenResponse.ok) {
        throw new Error(`Failed to fetch id token from Cognito: ${stringify(await idTokenResponse.json(), null, 2)}`);
    }
    ...
    const params = {
        ...
        Payload: stringToBytes(stringify({
            accessToken,
            idToken,
        })),
    };
