...
export async function getAllListings(options) {
    ...
    const [rawListings, latestBlock] = await Promise.all([
        getAllInBatches((startId, endId) => getAllListingGenerated({ contract: options.contract, endId, startId }), {
            end,
    ...
    const listings = (await Promise.all(rawListings.map((rawListing) => mapDirectListing({
