...
export async function getAllAuctions(options) {
    ...
    const [rawAuctions, latestBlock] = await Promise.all([
        getAllInBatches((startId, endId) => getAllAuctionsGenerated({ contract: options.contract, endId, startId }), {
            end,
    ...
    const auctions = (await Promise.all(rawAuctions.map((rawAuction) => mapEnglishAuction({
        contract: options.contract,
        latestBlock,
        rawAuction,
    }).catch(() => null)))).filter((auction) => auction !== null);
