...
export async function getClaimConditions(options) {
    try {
        if (options.singlePhaseDrop) {
            return SinglePhase.claimCondition(options).then(([startTimestamp, maxClaimableSupply, supplyClaimed, quantityLimitPerWallet, merkleRoot, pricePerToken, currency, metadata,]) => [
                {
                    currency,
                    maxClaimableSupply,
                    merkleRoot,
