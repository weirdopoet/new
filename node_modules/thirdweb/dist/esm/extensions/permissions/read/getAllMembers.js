import { getRoleMember, isGetRoleMemberSupported, } from "../__generated__/IPermissionsEnumerable/read/getRoleMember.js";
import { getRoleMemberCount, isGetRoleMemberCountSupported, } from "../__generated__/IPermissionsEnumerable/read/getRoleMemberCount.js";
 ...
 * ```
 */
export async function getAllRoleMembers(options) {
    ...
    for (let i = 0n; i < count; i++) {
        promises.push(getRoleMember({ contract: options.contract, index: i, role: roleHash }));
    }
 ...
 * ```
 */
export function isGetAllRoleMembersSupported(availableSelectors) {
    // both need to be supported to get all members
    return (isGetRoleMemberSupported(availableSelectors) &&
