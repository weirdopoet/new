...
export function WalletManagerScreen(props) {
    ...
    return (_jsxs(Container, { children: [_jsx(Container, { p: "lg", children: _jsx(ModalHeader, { onBack: props.onBack, title: "Wallets" }) }), _jsx(Line, {}), _jsx(Spacer, { y: "md" }), _jsx(Container, { px: "md", style: {
                    minHeight: "150px",
                }, children: _jsx(Container, { flex: "column", gap: "xs", children: connectedWallets
                        .filter((w) => !props.hiddenWallets?.includes(w.id))
                        .map((w) => {
                        return (_jsx(WalletManangerButton, { chain: props.activeChain, client: props.client, onClick: () => {
                                setActive(w);
                                props.onBack();
                            }, wallet: w }, w.id));
                    }) }) }), _jsx(Spacer, { y: "xxl" }), _jsx(Container, { p: "lg", children: !hideConnectButton && (_jsxs(Button, { fullWidth: true, gap: "xs", onClick: () => {
                        setScreen("connect");
                    }, variant: "accent", children: [_jsx(PlusIcon, { height: iconSize.sm, width: iconSize.sm }), "Connect Wallet"] })) })] }));
}
function WalletManangerButton(props) {
    ...
    return (_jsxs("div", { style: {
            position: "relative",
                ...
                }, children: _jsxs(Container, { center: "y", flex: "row", gap: "md", children: [_jsx(WalletImage, { client: props.client, id: walletId, size: iconSize.lg }), _jsxs(Container, { flex: "column", gap: "4xs", children: [_jsx(Text, { color: "primaryText", children: shortenAddress(address || "") }), balanceQuery.data ? (_jsx(Text, { color: "secondaryText", size: "xs", weight: 400, children: formatTokenBalance(balanceQuery.data) })) : (_jsx(Skeleton, { height: fontSize.sm, width: "100px" }))] })] }) }), _jsx("div", { style: {
                    position: "absolute",
                    right: spacing.xxs,
                    top: "50%",
                    transform: "translateY(-50%)",
                    zIndex: 1,
                }, children: activeWallet?.id === props.wallet.id ? (_jsx(ToolTip, { tip: "Active Wallet", children: _jsx(IconButton, { children: _jsx(CheckIcon, { height: iconSize.md, width: iconSize.md }) }) })) : (_jsx(ToolTip, { tip: "Disconnect", children: _jsx(IconButton, { onClick: () => {
                            disconnect(props.wallet);
                        }, children: _jsx(MinusIcon, { height: iconSize.md, style: {
