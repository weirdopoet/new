...
export function WalletImage(props) {
    ...
    if (image?.authProvider === "email") {
        return _jsx(EmailIcon, { color: theme.colors.accentText, size: props.size });
    }
    if (image?.authProvider === "phone") {
        return _jsx(PhoneIcon, { color: theme.colors.accentText, size: props.size });
    }
    if (image?.authProvider === "passkey") {
        return (_jsx(FingerPrintIcon, { color: theme.colors.accentText, size: props.size }));
    }
    if (image?.authProvider === "wallet") {
        return (_jsx(OutlineWalletIcon, { color: theme.colors.accentText, size: props.size }));
    }
    if (image?.authProvider === "guest") {
        return _jsx(GuestIcon, { color: theme.colors.accentText, size: props.size });
    }
    if (image?.uri) {
        return (_jsx(Img, { client: props.client, height: props.size, loading: "eager", src: image.uri, style: {
                borderRadius: radius.md,
                ...props.style,
            }, width: props.size }));
    }
    return (_jsx(WalletImageQuery, { client: props.client, id: props.id, size: props.size }));
}
function WalletImageQuery(props) {
    const walletImage = useWalletImage(props.id);
    if (walletImage.isFetched && !walletImage.data) {
        return (_jsx(Img, { client: props.client, height: props.size, src: genericWalletIcon, width: props.size }));
    }
    return _jsx(OutlineWalletIcon, { size: props.size });
}
