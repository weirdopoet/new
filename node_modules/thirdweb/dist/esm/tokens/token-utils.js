...
import { DEFAULT_DEVELOPER_REWARD_BPS, DEFAULT_MAX_SUPPLY_ERC20, DEFAULT_POOL_INITIAL_TICK, } from "./constants.js";
...
export async function encodeInitParams(options) {
    ...
    return encodeInitialize({
        ...
        maxSupply: toUnits(params.maxSupply.toString() || DEFAULT_MAX_SUPPLY_ERC20.toString(), 18),
    });
}
export function encodePoolConfig(poolConfig) {
    const POOL_PARAMS = [
        {
            name: "amount",
            type: "uint256",
...
export function generateSalt(salt, flags = SaltFlag.MIX_SENDER) {
    /*
    ...
    if (salt && isHex(salt)) {
        ...
        if (hex.length === 64) {
            ...
            salt = `0x${keccak256(toBytes(hex)).slice(4)}`;
        }
    }
    else if (salt && !isHex(salt)) {
        // 31 bytes of salt data
        salt = `0x${keccak256(toBytes(salt)).slice(4)}`;
    }
