...
export const CONTRACT_PUBLISHER_ADDRESS = "0xf5b896Ddb5146D5dA77efF4efBb3Eae36E300808"; // Polygon only
 ...
 * @internal
 */
export async function fetchDeployBytecodeFromPublishedContractMetadata(contract) {
    ...
    const pubmeta = await Promise.all(publishedMetadataUri
        ...
        .map((uri) => download({ client: contract.client, uri })
        .then((res) => res.text())
        .then(JSON.parse)));
    return pubmeta.length > 0
        ? await (await download({ client: contract.client, uri: pubmeta[0].bytecodeUri })).text()
        : undefined;
}
 ...
 * @contract
 */
export async function fetchPublishedContract(options) {
    const contractPublisher = getContract({
        address: CONTRACT_PUBLISHER_ADDRESS,
        chain: polygon,
        client: options.client,
