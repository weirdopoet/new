 ...
 * @internal
 */
export async function fetchConstructorParams(options) {
    ...
    try {
        const bytecode = await fetchDeployBytecodeFromPublishedContractMetadata(options.contract);
        if (!bytecode) {
            throw new Error("Contract not published through thirdweb");
        }
        const bytecodeHex = bytecode.startsWith("0x") ? bytecode : `0x${bytecode}`;
        const create2FactoryAddress = await computeCreate2FactoryAddress(options.contract);
