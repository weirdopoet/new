...
var evm_wallets_base_dist_thirdwebDevWalletsEvmWalletsBase = require('../../../../dist/base-23fd36a9.cjs.dev.js');
...
require('../../../../dist/headers-2be346e5.cjs.dev.js');
...
class SmartWallet extends evm_wallets_base_dist_thirdwebDevWalletsEvmWalletsBase.AbstractClientWallet {
  ...
  async getConnector() {
    if (!this.connector) {
      if (this.options && (await utils.isZkSyncChain(this.options.chain, this.options.clientId, this.options.secretKey))) {
        const {
          ZkSyncConnector
        } = await Promise.resolve().then(function () { return require('../../../../dist/zk-connector-272a7c5c.cjs.dev.js'); });
  ...
  async isDeployed() {
    const connector = await this.getConnector();
    if (connector.chainId && (await utils.isZkSyncChain(connector.chainId))) {
      return true;
    }
  ...
  async verifySignature(message, signature, address, chainId) {
    ...
    if (chainId !== undefined) {
      try {
        return await utils.checkContractWalletSignature(message, signature, address, chainId || 1, this.options?.clientId, this.options?.secretKey);
      } catch {
        // no-op
      }
    }
